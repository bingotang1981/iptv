...
func setupRouter(adurl string) *gin.Engine {
	...
	r.GET("/tv.m3u", func(c *gin.Context) {
		if filePath == "" {
			...
			c.Writer.Header().Set("Content-Type", "application/octet-stream")
			c.Writer.Header().Set("Content-Disposition", "attachment; filename=tv.m3u")
			mainm3uobj.GetMainM3u(c.Writer, filePath+"/tv.m3u")
		}
	})
	...
	r.GET("/:path/:rid", func(c *gin.Context) {
		...
		case "tptv":
			...
			if ts == "" {
				...
				tptvobj.HandleTsRequest(c.Writer, ts)
			}
		case "douyin":
...
func main() {
	...
	fmt.Println("M3U8FileFolderPath: " + filePath)

	defurl := "http://tv.ximix.us:88/test.m3u8"
	r := setupRouter(defurl)
	r.Run(":35455")
}
